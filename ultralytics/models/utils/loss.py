...
class RTDETRDetectionLoss(DETRLoss):
    ...
    def get_dn_match_indices(dn_pos_idx, dn_num_group, gt_groups):
        ...
        # Initialize tensors for indices
        dn_match_indices = [None] * len(gt_groups)
        idx_groups = torch.cumsum(torch.tensor([0] + gt_groups[:-1]), 0)

        # Vectorized operations for gt_idx construction
        for i, num_gt in enumerate(gt_groups):
            if num_gt > 0:
                gt_idx = torch.arange(num_gt, dtype=torch.long) + idx_groups[i]
                gt_idx = gt_idx.repeat(dn_num_group)
                pos_idx_len = len(dn_pos_idx[i])
                
                assert pos_idx_len == len(gt_idx), f"Expected the same length, but got {pos_idx_len} and {len(gt_idx)} respectively."
                dn_match_indices[i] = (dn_pos_idx[i], gt_idx)
            else:
                dn_match_indices[i] = (torch.zeros([0], dtype=torch.long), torch.zeros([0], dtype=torch.long))
